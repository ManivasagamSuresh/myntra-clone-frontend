...
import { ChangePrice, RemovecartR, RemoveTotalPrice, TotalPrice } from '../redux/Userslice';
...
let RemoveCart = async()=>{
  try {
    let remove = await axios.put(`${Config.api}/cartIdRemove/${currentUser.others._id}`,{pId:prod},
    {headers:{"Authorization":localStorage.getItem("accessToken")}});
      ...
      <div className="col-md-8">
        <div className="card-body">
          ...
          <div className='cartcard-qty'>Qty :
            <select className='cartcard-qtyvalue' value={Qty} onChange={(e)=>{setQty(e.target.value) }}>
            <option > 1</option>
            <option >2</option>
            <option>3</option>
            <option>4</option>
