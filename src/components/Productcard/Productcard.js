...
function Productcard({product,Wish}) {
  ...
  let Addwishlist = async()=>{
    try {
      ...
      // console.log(add);
      // let wish = await axios.post(`${Config.api}/wishlist`,add,
      // {headers:{"Authorization":localStorage.getItem("accessToken")}}
      // )
      let wishid = await axios.put(`${Config.api}/wishId/${currentUser.others._id}`,add,
      {headers:{"Authorization":localStorage.getItem("accessToken")}}
      )
  ...
  useEffect(()=>{
    // const a = Wish.findIndex((ele)=>ele.pId==product._id);
  ...
  return (
    <div className='col-lg-3 productcard'  onMouseEnter={()=>{setWishlist(true)}} onMouseLeave={()=>{setWishlist(false)}}>
      {Wishlist ? <div className='productcard-wishlist' >{Wished?<button className='productcard-wishlistbutton'><BsFillHeartFill style={{color:"red"}}/> Wishlisted</button>:<button type='button' className='productcard-wishlistbutton' onClick={()=>{Addwishlist();}}><CiHeart/> Wishlist</button>}</div> : null}
      <img src={`${product.img}`} className="productcard-img" onClick={()=>{navigate(`/product/${product._id}`)}} />
