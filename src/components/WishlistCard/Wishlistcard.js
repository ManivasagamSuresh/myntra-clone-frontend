...
let Removewish = async()=>{
  try {
    ...
    // {headers:{"Authorization":localStorage.getItem("accessToken")}});
    let removeId = await axios.put(`${Config.api}/wishIdRemove/${currentUser.others._id}`,{pId:prod},
    {headers:{"Authorization":localStorage.getItem("accessToken")}});
...
let Addcart = async()=>{
  ...
  return (
    <div className='col-lg-3 Wishlistcard' >
      <div className='wishlistcard-remove' onClick={()=>{Removewish()}}><span className='wishlistcard-removeX'>x</span></div>
